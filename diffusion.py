...
class StableDiffusion(nn.Module):
    ...
    def __init__(self, device, version):
        ...
        elif self.sd_version == 'customer':
            self.use_depth = True
            self.model_key = '/home/TeCH/exp/examples/name/sd_model'
            # self.model_key = '/home/depth-adapted-sd1.5'
        ...
        # 加载 CLIP 的图像处理器和图像编码器
        self.processor = CLIPProcessor.from_pretrained(self.model_key)
        self.image_encoder = CLIPVisionModel.from_pretrained(self.model_key).to(self.device)
        ...
        self.controlnet = ControlNetModel.from_pretrained("lllyasviel/sd-controlnet-canny").to(self.device)
    ...
    def prompt_to_img(self, prompts, height=512, width=512, num_inference_steps=50, guidance_scale=7.5, latents=None,is_text=True):
        ...
        if is_text:
            ...
            img_embeds = self.get_img_embeds(prompts)
...
if __name__ == '__main__':
    ...
    # prompt_ = "a sks man, dark brown short and slightly messy hair, neutral, slightly frowning, sks light grey plain, casual T-shirt, sks grey regular fit jeans pants, hands are slightly spread out and relaxed" 
    # prompt_ = prompt_.replace('sks',"")
    prompt_ = "an White vest, sleeveless, open"
    prompt_ = Image.open("/home/color_000001.png").convert("RGB")
    imgs = []
    # fix_randomness(42)

    for _ in range(4):
        img = sd.prompt_to_img(prompt_, 512, 512, opt.steps,is_text=False)
        img = img / 255.
