...
class DMTetMesh(nn.Module):
    ...
    def sdf_loss(self,sample_num):
        indices = torch.randint(0, self.tet_v.shape[0], (sample_num,))
        sample_points = self.tet_v[indices]
    ...
    def set_sdf(self,mesh_v,mesh_f):
        ...
        import mesh_to_sdf
        sdf_tet = torch.tensor(mesh_to_sdf.mesh_to_sdf(mesh, self.tet_v.cpu().numpy()), dtype=torch.float32).to(self.device)
        self.gt_sdf = sdf_tet
