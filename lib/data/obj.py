...
class Mesh():
    ...
    def __init__(self, v=None, f=None, vn=None, fn=None, vt=None, ft=None, albedo=None, device=None, base=None, split=False):
        ...
        self.ref_v = None
        # only support a single albedo, (W, H, 3)
        self.albedo = albedo
        self.device = device
    ...
    # load from obj file
    @classmethod
    def load_obj(cls, path, albedo_path=None, device=None, init_empty_tex=False, use_vertex_tex=False, albedo_res=2048, ref_path=None, keypoints_path=None, init_uv=True,scale_data=None):
        mesh = cls()
        ...
        if scale_data is not None:
            mesh.auto_size_by_body(scale_data)
        else:
            mesh.auto_size()
    ...
    def auto_size_by_body(self,scale_date):  # to [-0.5, 0.5]
        scale = scale_date['scale']
        resize_matrix_inv = scale_date['resize_matrix_inv']
        offset = scale_date['offset']
        self.v = self.v - offset
        self.v = self.v * scale
        self.resize_matrix_inv = resize_matrix_inv
        self.scale = scale
        self.offset = offset
    ...
    def restore_size(self):
        # 恢复原始大小
        if self.resize_matrix_inv is None:
            raise ValueError("The mesh has not been normalized. Please call auto_size() first.")
        
        # 增加第四维度 1 来进行矩阵变换
        v_homogeneous = torch.cat([self.v, torch.ones(self.v.shape[0], 1, device=self.device)], dim=1)
    ...
    def export(self, path):
        ...
        trimesh_mesh = trimesh.Trimesh(vertices=v, faces=f)
