...
from .model.network_utils import get_encoder
from .data.obj import Mesh, safe_normalize
from .utils.tet_utils import build_tet_grid
from .model.dmtet_network import DMTetMesh
from .model.flexicube_network import FlexiMesh
from .model.color_network import ColorNetwork
from .deformer.def_util import deform_mesh
from .utils.uv_utils import texture_padding
...
class Renderer(nn.Module):
    ...
    def __init__(
        ...
        if cfg.model.use_dmtet_network:
            ...
            if hasattr(self.mesh,'keypoints') and self.mesh.keypoints is not None:
                self.keypoints = self.mesh.keypoints
            else:
                self.keypoints = None
            self.marching_tets = None
